...
def extract(file):
    ...
    # 保留前top_num个的特征
    top_num = 200
    # 计算所有短序列的词频,以及出现次数最高的前top_num个字节码短序列
    byte_topnum_feature_dict, byte_all_feature_dict = countByteDF(paths, N, top_num)
    # 计算所有短序列的词频,以及出现次数最高的前top_num个字节码短序列
    op_topnum_feature_dict, op_all_feature_dict = countOpDF(paths, N, top_num)
    # 统计文件总数
    sum_of_file = 0
    for path in paths:
    ...
    features.extend(countByteTFplusIDF(file, sum_of_file, byte_topnum_feature_dict, byte_all_feature_dict, N))
    # 将PE文件的操作码特征提取并加入，也是一个1Xtop_num的一维向量
    features.extend(countOpTFplusIDF(file, sum_of_file, op_topnum_feature_dict, op_all_feature_dict, N))
    return features
